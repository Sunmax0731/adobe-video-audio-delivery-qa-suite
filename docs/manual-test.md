...
- 作業ディレクトリ: `D:\AI\AdobePlugin\PremierePro\adobe-video-audio-delivery-qa-suite`
...
```powershell
cd D:\AI\AdobePlugin\PremierePro\adobe-video-audio-delivery-qa-suite
npm test
npm start
```
...
5. Premiere Pro panel concept + evidence CLI の UI または配布面を開き、検証結果をユーザーが次アクションに移せる粒度で確認する。
...
- docs ZIP が `dist/adobe-video-audio-delivery-qa-suite-docs.zip` に存在する。
